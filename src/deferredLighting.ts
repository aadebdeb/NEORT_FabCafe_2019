...
export class DeferredLighting {
  ...
  constructor(gl: WebGL2RenderingContext) {
    ...
    this.program = new Program(gl, fillViewportVertexShader, deferredLightingFragmentShader,
      ['u_gBufferTexture0', 'u_gBufferTexture1', 'u_gBufferTexture2', 'u_gBufferTexture3', 'u_cameraPos', 'u_time']);
  }
  ...
  apply(gl: WebGL2RenderingContext, gBuffer: GBuffer, camera: Camera, elapsedSecs: number) {
    ...
    setUniformTexture(gl, 1, gBuffer.colorTexture1, this.program.getUniform('u_gBufferTexture1'));
    setUniformTexture(gl, 2, gBuffer.colorTexture2, this.program.getUniform('u_gBufferTexture2'));
    setUniformTexture(gl, 3, gBuffer.colorTexture3, this.program.getUniform('u_gBufferTexture3'));
